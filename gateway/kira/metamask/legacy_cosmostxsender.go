...
func legacySendTx(txRawData string, gwCosmosmux *runtime.ServeMux, r *http.Request) (string, error) {
	...
	txBytes, err = legacySignTx(ethTxData, gwCosmosmux, r, msgType)
...
func legacyGetStructHash(txType int, valParam string) ethcommon.Hash {
	abiPack := abi.ABI{}
...
func legacyValidateEIP712Sign(v, r, s []byte, sender ethcommon.Address, valParam string, txType int) bool {
	abiPack := abi.ABI{}
	...
	eip712DomainSeparatorABIPack, _ := PackABIParams(abiPack,
		...
		convertByteArr2Bytes32(uint32To32Bytes(config.DefaultChainID)),
	)
	eip712DomainSeparator := crypto.Keccak256Hash(eip712DomainSeparatorABIPack)

	// get StructHash
	structHash := legacyGetStructHash(txType, valParam)
...
func legacySignTx(ethTxData EthTxData, gwCosmosmux *runtime.ServeMux, r *http.Request, txType int) ([]byte, error) {
	...
	if txType != MsgBankSend {
		...
		validation := legacyValidateEIP712Sign(params[0][len(params[0])-1:], params[1], params[2], ethcommon.BytesToAddress(params[3][12:]), valParam, txType)
		if !validation {
