...
func serveFaucet(r *http.Request, gwCosmosmux *runtime.ServeMux, request types.InterxRequest, rpcAddr string, bech32addr string, token string) (interface{}, interface{}, int) {
	...
	if err != nil {
		log.CustomLogger().Error("[faucet] Invalid bech32addr: ", config.Config.Faucet.Address)
	...
	if err != nil {
		log.CustomLogger().Error("[faucet] Invalid bech32addr: ", claimAccAddr)
	...
	if timeLeft > 0 {
		log.CustomLogger().Error("[faucet] Claim time left: ", timeLeft)
	...
	if !ok {
		log.CustomLogger().Error("[faucet] Failed to get faucet amount from the configuration")
	...
	if !ok {
		log.CustomLogger().Error("[faucet] Failed to get faucet minimum amount from the configuration")
		return common.ServeError(102, "", "invalid token", http.StatusBadRequest)
	}
	...
	if !ok {
		log.CustomLogger().Error("[faucet] Failed to get fee amount from the configuration")
		return common.ServeError(102, "", "invalid token", http.StatusBadRequest)
	}
	...
	if err != nil {
		log.CustomLogger().Error("[faucet] Failed to parse fee amount from the configuration: ", coinStr)
		return common.ServeError(102, "", "invalid token", http.StatusBadRequest)
	}

	// log.CustomLogger().Info("[faucet] Available amount: ", availableAmount)
	// log.CustomLogger().Info("[faucet] Claim amount: ", claimAmount)
	// log.CustomLogger().Info("[faucet] Faucet amount: ", faucetAmount)
	// log.CustomLogger().Info("[faucet] Faucet minimum amount: ", faucetMininumAmount)

	if faucetAmount.Cmp(claimAmount) <= 0 {
		log.CustomLogger().Error("[faucet] No need to send tokens: faucetAmount <= claimAmount")
	...
	if claimingAmount.Cmp(remainingAmount) > 0 {
		log.CustomLogger().Error("[faucet] Not enough tokens: faucetAmount-claimAmount > availableAmount-faucetMininumAmount")
	...
	// log.CustomLogger().Info("[faucet] accountNumber: ", accountNumber)
	// log.CustomLogger().Info("[faucet] sequence: ", sequence)
	...
	if err != nil {
		log.CustomLogger().Error("[faucet] Failed to sign transaction: ", err)
		panic(err)
	}
	...
	if err != nil {
		log.CustomLogger().Error("[faucet] Failed to set SignatureV2: ", err)
	...
	if err != nil {
		log.CustomLogger().Error("[faucet] Failed to get tx bytes: ", err)
	...
	if err != nil {
		log.CustomLogger().Error("[faucet] Failed to broadcast transaction: ", err)
